...
def _body(request):
    ...
    if request.content_type.startswith('application/json') and request.body:
        ...
        # Ensure we capture multiple values for parent_users if present
        if 'parent_users' in request.POST:
            data['parent_users'] = request.POST.getlist('parent_users')

    if isinstance(data, dict):
        if 'user_id' in data and 'user' not in data:
            data['user'] = data['user_id']
        if 'parent_user_id' in data and 'parent_users' not in data:
            data['parent_users'] = [data['parent_user_id']]
        elif 'parent_user' in data and 'parent_users' not in data:
            data['parent_users'] = [data['parent_user']]
...
def _can_view_student_sensitive(user, student):
    ...
    if student.user_id == user.id:
        return True
    if student.parent_users.filter(id=user.id).exists():
        return True
...
def serialize_student_full(s, request=None):
    ...
    return {
        ...
        'parent_user_id': s.parent_users.first().id if s.parent_users.exists() else None,
        'parent_users_ids': list(s.parent_users.values_list('id', flat=True)),
...
def student_list(request, institution_pk=None):
    ...
    if request.method == 'GET':
        ...
        if attendance_date_str:
            ...
            try:
                att_date = datetime.datetime.strptime(attendance_date_str, '%Y-%m-%d').date()
                student_users = [s['user_id'] for s in payload['results'] if s.get('user_id')]
                ...
                if student_users:
                    ...
                    recent_atts = Attendance.objects.filter(user_id__in=student_users, date__in=last_5_days)
                    recent_map = {}
                    for ra in recent_atts:
                        if ra.user_id not in recent_map:
                            recent_map[ra.user_id] = {}
                        recent_map[ra.user_id][ra.date] = ra.status
                    ...
                    for s in payload['results']:
                        ...
                        for d in last_5_days:
                            s['recent_attendance'].append({
                                'date': d.strftime('%d %b'),
                                'status': recent_map.get(user_id, {}).get(d) if user_id else None
                            })
                
                # Overall Stats for the whole query (not just this page)
                all_student_users = qs.values_list('user_id', flat=True)
                daily_atts = Attendance.objects.filter(user_id__in=all_student_users, date=att_date)
                total_present = daily_atts.filter(status__in=['Present', 'P']).count()
                total_absent = daily_atts.filter(status__in=['Absent', 'A']).count()
                
                payload['attendance_summary'] = {
                    'total_present': total_present,
                    'total_absent': total_absent,
                    'total_students': payload['count']
                }
            except Exception:
                pass
...
def my_children(request):
    """For a logged-in Parent — list every Student where parent_users contains them."""
    qs = (
        Student.objects.prefetch_related('parent_users').select_related('user', 'institution')
        .filter(parent_users=request.user)
...
def create_institution_student(request):
    ...
    try:
        ...
        parent_users_list = data.get('parent_users')
        if not parent_users_list and parent_user_id:
            parent_users_list = [parent_user_id]

        parent_user_objs = []
        if parent_users_list:
            if isinstance(parent_users_list, str):
                parent_users_list = [parent_users_list]
            for puid in parent_users_list:
                try:
                    pobj = User.objects.get(id=int(puid))
                    parent_user_objs.append(pobj)
        ...
        if parent_user_objs:
            student.parent_users.set(parent_user_objs)
