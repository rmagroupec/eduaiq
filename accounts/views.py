...
def parent_dashboard_view(request):
    # Sirf Parent role wale yahan aa sakte hain
    ...
    if child:
        ...
        from courses.models import AssignmentSubmission, QuizAttempt
        ...
        # Attendance
        all_attendances = Attendance.objects.filter(user=child.user)
        ...
        # Recent 5 days (Streak)
        today = timezone.localdate()
        start_date = today - datetime.timedelta(days=4)
        recent_qs = all_attendances.filter(date__gte=start_date, date__lte=today)
        att_dict = {a.date: a.status for a in recent_qs}
        ...
        for i in range(4, -1, -1):
            ...
            recent_logs.append({
                'date': d,
                'status': att_dict.get(d)
            })
            
        context['recent_logs'] = recent_logs
        
        # Exam Results / Academic Performance
        quiz_attempts = QuizAttempt.objects.filter(student=child.user).order_by('-started_at')[:10]
        context['exam_results'] = quiz_attempts
        
        # Fees
        invoices = Invoice.objects.filter(student=child.user).order_by('-created_at')[:5]
        context['invoices'] = invoices
        
        # Assignments
        assignments = AssignmentSubmission.objects.filter(student=child.user).order_by('-submitted_at')[:10]
...
def parent_exam_results_view(request):
    ...
    if child_id:
        ...
        child = children.first()
    context = {'child': child, 'children': children}

    if child:
        from courses.models import QuizAttempt
        from olympiad.models import OlympiadRegistration
        
        quiz_attempts = QuizAttempt.objects.filter(student=child.user).order_by('-started_at')
        context['quiz_attempts'] = quiz_attempts
        
        olympiad_regs = OlympiadRegistration.objects.filter(student=child.user).order_by('-registered_at')
        context['olympiad_regs'] = olympiad_regs
        
        total_graded = quiz_attempts.filter(status='graded').count()
        passed_quizzes = quiz_attempts.filter(passed=True).count()
        pass_rate = (passed_quizzes / total_graded * 100) if total_graded > 0 else 0
        
        context['total_graded'] = total_graded
        context['pass_rate'] = round(pass_rate, 1)

    return render(request, 'parent_exam_results.html', context)
...
def parent_attendance_view(request):
    ...
    if child:
        ...
        context['all_attendances'] = all_attendances

    return render(request, 'parent_attendance.html', context)

@login_required
def parent_assignments_view(request):
    if getattr(request.user, 'role', '') != 'parent':
        return redirect('/')
    
    # Debug: Check if parent has any children linked
    children = request.user.children.all()
    print("DEBUG: Parent's children queryset:", children)
    print("DEBUG: Number of children:", children.count())

    # Get specific child from URL param or default to first
    child_id = request.GET.get('child_id')
    if child_id:
        child = children.filter(id=child_id).first()
    else:
    ...
    if child:
        ...
        enrolled_course_ids = Enrollment.objects.filter(student=child.user).values_list('course_id', flat=True)
        
        # Get all assignment lessons for these courses
        ...
        submissions = AssignmentSubmission.objects.filter(student=child.user, lesson__in=assignment_lessons)
        submission_map = {sub.lesson_id: sub for sub in submissions}
        ...
        for lesson in assignment_lessons:
            sub = submission_map.get(lesson.id)
            if sub:
                status = sub.status
                score = sub.grade
        ...
        # Sort by date (unsubmitted at top, or sort by latest submitted)
        # We will sort unsubmitted first (None), then by submitted_at descending
        assignment_data.sort(key=lambda x: (x['submitted_at'] is not None, x['submitted_at']), reverse=True)
        
        context['assignments'] = assignment_data
        context['stats'] = {
            'total': len(assignment_lessons),
            'graded': graded_count,
            'pending': pending_grade_count
        }

    return render(request, 'parent_assignments.html', context)
